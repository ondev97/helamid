(function () {
  ...
  app.controller("myPageCtrl", function ($scope, $http) {
    ...
    $scope.menuScroll = function () {
      ($scope.header = $(".header")), $scope.header.addClass("active");
      setTimeout(function () {
      ...
      $scope.scrollNav = function (e) {
        ...
        $scope.pageAnimate = function (page_block) {
          $("html, body").animate(
            {
              scrollTop:
                $("." + page_block + "").offset().top - $scope.menuActiveHeight,
            },
            700
          );
        };
        if (
          $(e.target).hasClass("link_features") ||
          $(e.target).hasClass("container_arrow")
        ) {
          $scope.pageAnimate("elements");
        } else if ($(e.target).hasClass("link_offer")) {
          //$scope.pageAnimate('features');
          $scope.pageAnimate("ourPartner");
        } else if ($(e.target).hasClass("link_pricing")) {
          $scope.pageAnimate("features");
        } else if ($(e.target).hasClass("link_clients")) {
          $scope.pageAnimate("ondeclient");
    ...
    $scope.removeMobileVideo = function () {
      if (
        ...
        $(
          ".container, .features, .subscribe, .statistic, .video_preview, .promo, .contact"
    ...
    $scope.addTouchEventHandlers = function () {
      if (
        ...
        ng_click.each(function () {
          $(this)
            .attr("data-ng-touchstart", "scrollNav($event)")
    ...
    $scope.addDescriptionImage = function () {
      ...
      for (var i = 0; i < $scope.description_image.length; i++) {
        $scope.description_image
          .eq(i)
          .css(
            "background",
            "url(" +
              $scope.description_image.eq(i).attr("data-image") +
    ...
    $scope.setEffect = function () {
      ($scope.html = $("html")), ($scope.container = $(".container"));
      ...
      for (var i = 0; i < $('[data-ng-submit="mailchimpForm"]').length; i++) {
        (function (i) {
          ...
          $scope.form[i].ajaxChimp({
            ...
            url:
              "http://google.us3.list-manage.com/subscribe/post?u=54cac12d99d1b2a0c0e0177b4&amp;id=d5469b7ba3", // Replace this with your own mailchimp post URL. Don't remove the "". Just paste the url inside "".
          });
        })(i);
      }
    };
